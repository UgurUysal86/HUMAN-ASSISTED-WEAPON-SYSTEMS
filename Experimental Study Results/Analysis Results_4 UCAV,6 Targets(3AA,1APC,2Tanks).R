...
Data = Experimentation_Hypothesis_1_Results <- read_excel("C:/Users/Ugur/OneDrive/Studium UCF/18 Fall - IDS6916/Experiments/Experimentation Results_4 UCAV,6 Targets(3AA,1APC,2Tanks).xlsx", 
                                                          sheet = "T.Test Data")
...
leveneTest(y1, group, center = mean) #p-value 0.9122 --> var.equal = TRUE
...
t.test(Sample1, Sample2, var.equal = TRUE) # p-value = 0.4648 (No significant difference)
#############
...
leveneTest(y1, group, center = mean) # p-value 0.4928 --> var.equal = TRUE

# t.test (if p> 0.05 we conclude that there is not enough evidence of a difference between the (true) averages of the two samples at a=0.95)
t.test(Sample3, Sample4, var.equal = TRUE) # p-value = 0.1526 (No significant difference)
#############
...
leveneTest(y1, group, center = mean) #p-value 0.07992 --> var.equal = TRUE

# t.test (if p> 0.05 we conclude that there is not enough evidence of a difference between the (true) averages of the two samples at a=0.95)
t.test(Sample5, Sample6, var.equal = TRUE) # p-value = 0.0005219 (significant difference)
...
leveneTest(y1, group, center = mean) #p-value 0.943 --> var.equal = TRUE

# t.test (if p> 0.05 we conclude that there is not enough evidence of a difference between the (true) averages of the two samples at a=0.95)
t.test(Sample7, Sample8, var.equal = TRUE) # p-value = 0.5808 (No significant difference)
...
Sample9  = Data$`Only hostile targets engaged {True = 1, False = 0} - A`
Sample10 = Data$`Only hostile targets engaged {True = 1, False = 0} - B`
...
#Levene's Test:
# p-value greater than 0.05, indicating that there is no significant difference in variances between the groups
y1 <- c(Sample9, Sample10)
group <- as.factor(c(rep(1, length(Sample9)), rep(2, length(Sample10))))
leveneTest(y1, group, center = mean) #p-value 0.002985 --> var.equal = FALSE

# t.test (if p> 0.05 we conclude that there is not enough evidence of a difference between the (true) averages of the two samples at a=0.95)
t.test(Sample9, Sample10, var.equal = FALSE) # p-value = 0.1608 (No significant difference)
...
leveneTest(y1, group, center = mean) # p-value 0.3998 --> var.equal = TRUE

# t.test (if p> 0.05 we conclude that there is not enough evidence of a difference between the (true) averages of the two samples at a=0.95)
t.test(Sample13, Sample14, var.equal = TRUE) # p-value = 3.209e-10 (significant difference)
#############
